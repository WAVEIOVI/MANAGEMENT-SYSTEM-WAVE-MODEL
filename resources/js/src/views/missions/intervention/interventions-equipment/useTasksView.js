...
export default function useTasksView() {
  ...
  const tableColumnsTasks = [
    ...
    { key: 'equipment_intervention_status', label: 'Status' },
  ...
  const fetchInterventionTasks = (ctx, callback) => {
    store
      .dispatch('app-intervention/fetchInterventionTasks', {
        queryParams: {
  ...
  const fetchEquipmentTasks = dataItemId => {
    store.dispatch('app-intervention/fetchEquipmentTasks', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(response => {
  ...
  const removeEquipmentTasks = dataItemId => {
    store
      .dispatch('app-intervention/removeEquipmentTasks', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(() => {
        refetchTasksData()
      })
  }
