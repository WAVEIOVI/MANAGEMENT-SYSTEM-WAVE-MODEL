...
export default function useInterventionsList() {
  ...
  // Table Handlers
  const tableColumns = [
    { key: 'workshop_id', label: 'Intervention ID' },
    { key: 'customer', label: 'Customer', sortable: true },
    { key: 'workshop_date', label: 'Intervention Date' },
    { key: 'workshop_status', label: 'Status' },
    { key: 'actions' },
  ]
  ...
  const fetchWorkshops = (ctx, callback) => {
    store
      .dispatch('app-workshop/fetchWorkshops', {
      ...
      .then(response => {
        ...
        pendingWorkshop.value = response.data.data
        console.log('pendingWorkshop', pendingWorkshop)
      })
      .catch(() => {
        toast({
          component: ToastificationContent,
          props: {
            title: 'Error fetching workshops list',
  ...
  const removeIntervention = dataItemId => {
    store
      .dispatch('app-workshop/removeIntervention', { id: dataItemId })
      .then(() => {
        refetchData()
      })
  }
  ...
  store
    .dispatch('app-workshop/fetchCustomers')
    .then(response => {
