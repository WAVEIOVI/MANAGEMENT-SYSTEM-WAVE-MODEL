...
export default function useWorkshopsEdit() {
  // Use toast
  const toast = useToast()

  const workshopScrapData = ref(null)
  const workshopContactData = ref(null)
  const workshopContractData = ref(null)
  const workshopOrderData = ref(null)

  const refWorkshopScrapsListTable = ref(null)
  ...
  const refetchData = () => {
    ...
    refWorkshopScrapsListTable.value.refresh()
  }

  // ------------------------------------------------
  // Tap Scraps : Fetch Workshop's Scraps List
  // ------------------------------------------------
  const fetchWorkshopScraps = (ctx, callback) => {
    store
      .dispatch('app-scrap/fetchWorkshopScraps', { id: router.currentRoute.params.id })
      .then(response => {
  ...
  // ------------------------------------------------
  // Tap Scraps : Fetch Workshop Scrap
  // ------------------------------------------------
  const fetchWorkshopScrap = dataItemId => {
    store.dispatch('app-scrap/fetchWorkshopScrap', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(response => {
        workshopScrapData.value = response.data.data
      })
      .catch(error => {
        if (error.response.status === 404) {
          workshopScrapData.value = undefined
        }
      })
  }

  // ------------------------------------------------
  // Tap Scraps : remove Workshop Scrap
  // ------------------------------------------------
  const removeWorkshopScrap = dataItemId => {
    store
      .dispatch('app-scrap/removeWorkshopScrap', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(() => {
        refetchData()
      })
  }
  ...
  const fetchMissionContact = dataItemId => {
    store.dispatch('app-contact/fetchMissionContact', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(response => {
        workshopContactData.value = response.data.data
      })
      .catch(error => {
        if (error.response.status === 404) {
          workshopContactData.value = undefined
        }
      })
  }
  ...
  const fetchMissionContract = dataItemId => {
    store.dispatch('app-contract/fetchMissionContract', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(response => {
        workshopContractData.value = response.data.data
      })
      .catch(error => {
        if (error.response.status === 404) {
          workshopContractData.value = undefined
        }
      })
  }
  ...
  const fetchMissionOrder = dataItemId => {
    store.dispatch('app-order/fetchMissionOrder', { id: router.currentRoute.params.id, idloc: dataItemId })
      .then(response => {
        workshopOrderData.value = response.data.data
      })
      .catch(error => {
        if (error.response.status === 404) {
          workshopOrderData.value = undefined
        }
      })
  }
